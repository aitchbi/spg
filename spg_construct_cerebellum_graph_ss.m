% SPG_CONSTRUCT_CEREBELLUM_GRAPH_SS Constructs a 'single slice' 
% cerebellum graph, suitable for 2D display. 
%
% The construction of the full cerebellum graph, as presented in 
%
...
% 
% spg_construct_cerebellum_graph.
%
% 
% G = SPG_CONSTRUCT_CEREBELLUM_GRAPH_SS(sl)
%
% Input: 
% 'slNum' : slice number, an interger within the range 1-29. 
%  
% Output:
% 'G'        : Graph structure.
%
%
% Example:
% >> G = SPG_CONSTRUCT_CEREBELLUM_GRAPH_SS(15);
% >> gsp_plot_graph(G)
%
% ------------------------------------------------------------
...
function G = spg_construct_cerebellum_graph_ss(slNum)
...
mask_ss = squeeze(mask(:,slNum,:));

G.A = compute_adjacency(mask_ss,8);
G.W = G.A;
G.N = size(G.A,1);
...
[x,y,z] = ind2sub(size(mask_ss),find(mask_ss(:)));
G.coords = [x,y,z];
end
...
switch conn
    case 8 % 8 connectivity in 2D mask
        ni=[alli-1; alli; alli+1; alli+1];
        nj=[allj+1; allj+1 ; allj+1; allj];
        nk=[allk  ; allk  ; allk ; allk];
    case 6
        ni=[alli  ; alli+1; alli  ];
        nj=[allj+1; allj  ; allj  ];
        nk=[allk  ; allk  ; allk+1];
