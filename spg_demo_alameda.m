% SPG_DEMO_ALAMEDA Constructs signal-adapted system of of spectral
% kernels on the Alameda graph as presented in:
%
...
% >> SPG_DEMO_ALAMEDA('N_subbands',5)
%
% 3. Changing the number of subbands and using smoothed warping:
% >> SPG_DEMO_ALAMEDA('N_subbands',5,'warpType','smoothed')
%
% ------------------------------------------------------------
...
function spg_demo_alameda(varargin)
...
dummy = load('signal_set_alameda.mat');
signalSets{1}.signals = [dummy.Y_am,dummy.Y_noon,dummy.Y_pm];
fprintf(' done.\n')
pause(0.2)
...
% plots graph -------------------------------------------------------------
gsp_plot_graph(G)
set(gcf,'Position', [1040, 10, 350, 350],...
    'Name','Alameda graph (zoom to explore)',...
    'NumberTitle','off')
